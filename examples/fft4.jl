...
function test_fft_multiply(mygrid, X, k_norm, alpha, occ)
    A = TensorConvolution(mygrid, occ, k_norm, alpha)
    return A * X
end

x_fft = test_fft_multiply(mygrid, X, k_norm, alpha, occ)

function test_direct_multiply(mygrid, X, k_norm, alpha, occ)
    r =  mygrid[occ]
...
x_direct = test_direct_multiply(mygrid, X, k_norm, alpha, occ)
...
A = TensorConvolution(g, occ, k_norm, alpha)
XX = reinterpret(ComplexF64, X)
x_fft  = similar(XX)
mul!(x_fft, A, XX)
@btime  mul!(x_fft, A, X)


reinterpret(reshape, ComplexF64, X)
